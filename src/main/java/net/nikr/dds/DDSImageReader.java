...
public class DDSImageReader extends  ImageReader {
	...
	public Iterator<ImageTypeSpecifier> getImageTypes(int imageIndex) throws IIOException {
		...
		switch (ddsHeader.getPixelFormat().getFormat()) {
			/*
			case PixelFormat.FORMAT_DXT2:
			case PixelFormat.FORMAT_DXT4:
				imageType = ImageTypeSpecifier.createFromBufferedImageType(BufferedImage.TYPE_4BYTE_ABGR_PRE);
				break;
			 */
			case DXT1:
			case DXT3:
			...
			case A8R8G8B8:
				...
				break;
			default:	
				imageType = ImageTypeSpecifier.createFromBufferedImageType(BufferedImage.TYPE_3BYTE_BGR);
				break;
		}
		l.add(imageType);
		return l.iterator();
	...
	private void readHeader() throws IIOException {
		if (ddsHeader != null) return;
		
		if (stream == null) throw new IIOException("Failed to load header: Stream is null");
		...
		try {
			...
			if (magic != MAGIC) throw new IIOException("Failed To Load Header: MAGIC is not MAGIC");
			int size = stream.readInt();
			//System.out.println("size: "+size);
			if (size != 124)  throw new IIOException("Failed To Load Header: Size value is not right");
			...
			/*
			System.out.println((i++)+": "+stream.readInt());
			System.out.println((i++)+": "+stream.readInt());
			System.out.println((i++)+": "+stream.readInt());
			System.out.println((i++)+": "+stream.readInt());
			System.out.println((i++)+": "+stream.readInt());
			...
			System.out.println((i++)+": "+stream.readInt());
			*/
			PixelFormat ddPixelFormat = readPixelFormat();
			int caps = stream.readInt();
			int caps2 = stream.readInt();
			int caps3 = stream.readInt();
			int caps4 = stream.readInt();
			ddsHeader = new  DDSHeader(size, flags, height, width, linearSize, depth, mipMapCount, ddPixelFormat, caps, caps2, caps3, caps4);
	...
	private PixelFormat readPixelFormat() throws IOException {
		int size = stream.readInt();
		if (size != 32 ) throw new IOException("Failed load PixelFormat: File ill formed");  //should throw something...
		int flags = stream.readInt();
